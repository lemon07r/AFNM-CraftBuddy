...
- Status: available via `modAPI.utils.evaluateScaling` (`afnm-types@0.6.49`), but the current runtime provider is not trusted in search because it can diverge from hypothetical-state simulation
...
- Status: available via `modAPI.utils.calculateCraftingOvercrit` (`afnm-types@0.6.49`)
...
- Status: available via `modAPI.utils.canUseAction` (`afnm-types@0.6.49`)
...
- Status: available via `modAPI.utils.getMaxCompletion/getMaxPerfection` (`afnm-types@0.6.49`)
...
- Status: available via `modAPI.utils.completionBonusBuffName` (`afnm-types@0.6.49`)
...
- documented root-state access via `window.modAPI.subscribe(...)` / `window.modAPI.getGameStateSnapshot()`, now used as the primary crafting-session/store path
