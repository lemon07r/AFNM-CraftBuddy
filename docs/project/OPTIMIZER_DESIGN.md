...
- The guaranteed floor also applies to exact `1`-stability unmet-goal runway traps where an EV stabilize line can leave only a token guaranteed floor, but it does not automatically collapse sublime continuation lines once base success is already secured.
