...
| Overcraft extras are unilateral and gated on the secured tier; guaranteed-only live, fractional at terminal | Runtime-verified (RUNTIME_EVIDENCE §12): the game pays each bar's extra bands independently. Conjunctive `min` extras were the 297%-vs-1100% plateau. Fractional bonus-roll EV at *live* horizon leaves was measured to flip buff-setup lines on band-fraction noise, so live extras bank guaranteed bands only; *terminal* scoring prices the fraction, because finished crafts left every craft-ending action bit-identical and the qi tie-breaker preferred zero-gain enders (user-report-overcraft-endgame). Extras are bounded by the finish flat's band count, since the runtime clamps bars there. The soft overshoot penalty stays on in both modes because it is the only ranking signal between two overshooting live lines. |
