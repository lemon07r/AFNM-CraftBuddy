...
- Target game version is **0.7.12**: harmony is player-selected across eight types (including Captivating Cadence), outcome tiers are conjunctive band gates, there is no manual Finish Craft, the game applies its own crafting auto-use loadout before every technique, buffs can carry stateful `internalState` and triggered effects, Illume Crucible applies `sealedMaxStability`, discordant conditions shift condition distributions, Eccentric Decree penalizes straying by -15 harmony / -15 Qi Pool, Captivating Cadence builds non-repeating action chains with stat boosts and penalizes repeats by -50 harmony / -1 max stability, and the Completion Bonus buff grants +10 `perfectionBoost` per stack instead of control.
