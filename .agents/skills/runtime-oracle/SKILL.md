...
Use the installed AFNM bundle before trusting docs, types, historical notes, or screenshots. Current target: **0.7.6**.
...
4. **Removed APIs are findings too**: 0.7.5 deleted the item-type → harmony-type mapping utility and 0.7.6 has not brought it back (`hasItemTypeToHarmonyType` is still false). If a grep for a helper comes back empty, check whether the game dropped it before assuming a bad pattern.
